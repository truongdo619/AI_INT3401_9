...
def enhancedPacmanFeatures(state, action):
    ...
    closestFood = len(food) != 0 and min(map(lambda x: util.manhattanDistance(pacmanPosition, x), food)) or None
    features['foodNum'] = len(food)
    features['closestFood'] = closestFood and 1.0 / closestFood or 0

    ghosts = state.getGhostPositions()
    closestGhost = len(ghosts) != 0 and min(map(lambda x: util.manhattanDistance(pacmanPosition, x), ghosts)) or None
    features['ghostsNum'] = len(ghosts)
    features['closestGhost'] = closestGhost and 1.0 / closestGhost or 0

    capsules = state.getCapsules()
    closestCapsule = len(capsules) != 0 and min(map(lambda x: util.manhattanDistance(pacmanPosition, x), capsules)) or None
    features['capsulesNum'] = len(capsules)
    features['closestCapsule'] = closestCapsule and  1.0 / closestCapsule or 0
